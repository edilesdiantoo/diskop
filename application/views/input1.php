...
<body class="bg-color">
  ...
  <form class="" id="upload_form" method="post" enctype="multipart/form-data" action="<?= site_url() ?>TransaksiController/input2">
    <div class="container">
      <div class="row pt-4">
        <div class="col">
          <div class="mb-3">
  ...
  <script type="text/javascript">
    ...
    var oldValueNIK = '';
    var alertNIK = document.getElementById('alertNIK');
    document.getElementById('nik').onkeyup = function() {
      if (!textLength(this.value)) {
        this.value = oldValueNIK;
        alertNIK.innerHTML = 'max 16 karakter'
        // document.getElementById("nik").setAttribute("style", "border: 1px solid #198754");
      } else if (!textLength(this.value) < 16) {
        oldValueNIK = this.value;
        document.getElementById("nik").setAttribute("style", "border: 1px solid #dc3545;");
        var len = this.value.length;
        if (len <= 16) {
          var textku = ' karakter'
          $('#alertNIK').text(16 - len + ' Karakter');
          $('#niktest').val(16 - len);
        }
      } else oldValueNIK = this.value;
      var angka = document.getElementById('niktest').value
      if (parseInt(angka) == 0) {
