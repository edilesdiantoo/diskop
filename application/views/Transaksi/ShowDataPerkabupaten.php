                        ...
                        <div class="table-responsive">
                            <table id="" class="table">
                                ...
                                <tbody id="">
                                    ...
                                    $no = 1;
                                    $jumKab = 0;
                                    foreach ($getDataPerkab as $key => $value) { ?>
                                        <tr>
                                            <td><?= $no++; ?></td>
                                            <td><?= $value->kab ?></td>
                                            <td><?= $value->kab_usaha_total ?></td>
                                            <!-- <td><?= $value->mil_20 ?></td> -->
                                            <td><?= @$value->mil_5 ?></td>
                                            <!-- <td><?= $value->mil_10 ?></td> -->
                                            <!-- <td><?= $value->mak_10 ?></td> -->
                                        ...
                                        <input type="hidden" class="kab" value="<?= $value->kab_usaha_total ?>">
                                        <!-- <input type="hidden" class="mil_20" value="<?= $value->mil_20 ?>"> -->
                                        <input type="hidden" class="mil_5" value="<?= @$value->mil_20 ?>">
                                        <!-- <input type="hidden" class="mil_10" value="<?= $value->mil_10 ?>"> -->
                                        <!-- <input type="hidden" class="mak_10" value="<?= $value->mak_10 ?>"> -->
                                        <input type="hidden" class="mak_5" value="<?= $value->mak_5 ?>">
                                        <!-- <input type="hidden" class="wp_10" value="<?= $value->wp_10 ?>"> -->
                                        <input type="hidden" class="wp_5" value="<?= $value->wp_5 ?>">
                                    <?php
                                        // $jumKab += $value;
                                    } ?>
                                    <tr>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // Kab
        var valueArrKab = [];
        document.querySelectorAll('.kab').forEach(function(el) {
            valueArrKab.push(el.value);
        });
        var jumStringKab = valueArrKab.map(Number);
        var resultAllKab = jumStringKab.reduce((a, b) => a + b, 0)
        document.getElementById('sumKab').innerHTML = resultAllKab;
